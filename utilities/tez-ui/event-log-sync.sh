...
while [[ true ]]; do
    aws s3 sync $S3_LOG_URI/applications/$APPLICATION_ID/jobs/$JOB_RUN_ID/timeline-data/active $job_path/active --exclude '*$folder$*'
    # Hack to move done events to active folder as ATS doesnt read done path
    aws s3 sync $S3_LOG_URI/applications/$APPLICATION_ID/jobs/$JOB_RUN_ID/timeline-data/done $job_path/done_events/ --exclude '*$folder$*'
